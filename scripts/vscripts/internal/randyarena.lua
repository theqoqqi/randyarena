...
function RandyArena:_InitRandyArena()
  ...
  GameRules:SetCustomGameSetupAutoLaunchDelay(GAME_SETUP_AUTOLAUNCH_DELAY);
  ...
  DebugPrint('[RANDYARENA] GameRules set')

  --InitLogFile( "log/randyarena.txt","")
  ...
  ListenToGameEvent('dota_ability_channel_finished', Dynamic_Wrap(self, 'OnAbilityChannelFinished'), self)
  ...
  ListenToGameEvent('player_changename', Dynamic_Wrap(self, 'OnPlayerChangedName'), self)
  ListenToGameEvent('dota_rune_activated_server', Dynamic_Wrap(self, 'OnRuneActivated'), self)
  ListenToGameEvent('dota_player_take_tower_damage', Dynamic_Wrap(self, 'OnPlayerTakeTowerDamage'), self)
  ...
  ListenToGameEvent("dota_illusions_created", Dynamic_Wrap(self, 'OnIllusionsCreated'), self)
  ListenToGameEvent("dota_item_combined", Dynamic_Wrap(self, 'OnItemCombined'), self)
  ListenToGameEvent("dota_player_begin_cast", Dynamic_Wrap(self, 'OnAbilityCastBegins'), self)
  ListenToGameEvent("dota_tower_kill", Dynamic_Wrap(self, 'OnTowerKill'), self)
  ListenToGameEvent("dota_player_selected_custom_team", Dynamic_Wrap(self, 'OnPlayerSelectedCustomTeam'), self)
  ...
  --ListenToGameEvent("dota_tutorial_shop_toggled", Dynamic_Wrap(self, 'OnShopToggled'), self)

  --ListenToGameEvent('player_spawn', Dynamic_Wrap(self, 'OnPlayerSpawn'), self)
  --ListenToGameEvent('dota_unit_event', Dynamic_Wrap(self, 'OnDotaUnitEvent'), self)
  --ListenToGameEvent('nommed_tree', Dynamic_Wrap(self, 'OnPlayerAteTree'), self)
  --ListenToGameEvent('player_completed_game', Dynamic_Wrap(self, 'OnPlayerCompletedGame'), self)
  --ListenToGameEvent('dota_match_done', Dynamic_Wrap(self, 'OnDotaMatchDone'), self)
  --ListenToGameEvent('dota_combatlog', Dynamic_Wrap(self, 'OnCombatLogEvent'), self)
  --ListenToGameEvent('dota_player_killed', Dynamic_Wrap(self, 'OnPlayerKilled'), self)
  --ListenToGameEvent('player_team', Dynamic_Wrap(self, 'OnPlayerTeam'), self)
  ...
  local spew = 0
  if RANDYARENA_DEBUG_SPEW then
    spew = 1
  end
  Convars:RegisterConvar('randyarena_spew', tostring(spew), 'Set to 1 to start spewing randyarena debug info.  Set to 0 to disable.', 0)
  ...
  DebugPrint('[RANDYARENA] Done loading RandyArena randyarena!\n\n')
...
-- This function is called as the first player loads and sets up the RandyArena parameters
function RandyArena:_CaptureRandyArena()
  if mode == nil then
    -- Set RandyArena parameters
